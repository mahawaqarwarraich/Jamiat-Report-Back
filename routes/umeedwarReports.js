...
// Get specific day data for Umeedwar report
...
router.get('/day/:month/:year/:date', auth, async (req, res) => {
  try {
    ...
    console.error('Error fetching Umeedwar day data:', error);
    res.status(500).json({ message: 'Server error' });
  }
});

// Save or update QA data for Umeedwar report
...
router.post('/:month/:year/qa', auth, async (req, res) => {
  try {
    ...
    // Find the report for the current user with the given month and year
    let report = await UmeedwarReport.findOne({
      user: req.user._id,
      month,
      year
    });
    ...
    if (!report) {
      // Create new Umeedwar report
      const monthNames = ['January', 'February', 'March', 'April', 'May', 'June', 
                         'July', 'August', 'September', 'October', 'November', 'December'];
      const monthIndex = monthNames.indexOf(month);
      const daysInMonth = new Date(parseInt(year), monthIndex + 1, 0).getDate();

      report = new UmeedwarReport({
        user: req.user._id,
        month: month,
        year: year,
        days: []
      ...
      // Loop through all days of the month and create UmeedwarDay documents
      for (let i = 1; i <= daysInMonth; i++) {
        const umeedwarDay = new mongoose.Document({
          date: i,
          month: month,
          year: year
        }, UmeedwarDay);
        report.days.push(umeedwarDay);
      }
    ...
    console.error('Error saving Umeedwar QA data:', error);
    res.status(500).json({ message: 'Server error' });
  }
});

// Save or update day data for Umeedwar report
...
router.post('/:month/:year/:date', auth, async (req, res) => {
  try {
    ...
    if (!report) {
      ...
      // Create new Umeedwar report using Mongoose
      report = new UmeedwarReport({
        user: req.user._id,
        month: month,
        year: year,
      ...
      // Loop through all days of the month and create UmeedwarDay documents using Mongoose
      for (let i = 1; i <= daysInMonth; i++) {
        // Create new UmeedwarDay document using Mongoose (will use default values from schema)
        const umeedwarDay = new mongoose.Document({
          date: i,
          month: month,
          year: year
        ...
        // Push the UmeedwarDay document into report's days array
        report.days.push(umeedwarDay);
      }
      ...
      if (dayToUpdate) {
        ...
        umeedwarFields.forEach(fieldName => {
    ...
    console.error('Error saving Umeedwar day data:', error);
    res.status(500).json({ message: 'Server error' });
  }
});

// Get days array for Umeedwar report (for Monthly View)
// This route must come before /:month/:year to avoid route conflicts
...
// Get Umeedwar report for a specific month/year (for QA section and Monthly View)
router.get('/:month/:year', auth, async (req, res) => {
  try {
    ...
    console.error('Error fetching Umeedwar report:', error);
    res.status(500).json({ message: 'Server error' });
  }
});
